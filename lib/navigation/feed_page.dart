...
class FeedProvider extends ChangeNotifier {
  ...
  double latIncrement(userLat, userLong) {
    ...
    double degreeLatLength = earthRadius * pi / 180; // length of one degree of latitude at equator in miles
    double degreeLatLengthAtGivenLat = degreeLatLength * cos(latitude * pi / 180); // length of one degree of latitude at given latitude
    double latIncrement = miles / degreeLatLengthAtGivenLat; // latitude increment in degrees
    return latIncrement;
  }
   ...
   double longIncrement(userLat, userLong) {
    ...
    double degreeLatLengthAtGivenLat = degreeLatLength * cos(latitude * pi / 180); // length of one degree of latitude at given latitude
    double latIncrement = miles / degreeLatLengthAtGivenLat; // latitude increment in degrees
    double lonIncrement = latIncrement * cos(latitude * pi / 180); // longitude increment in degrees
    return lonIncrement;
  }
  ...
  Future<void> fetchInitial(int limit) async {
      ...
      final snapshot = await FirebaseFirestore.instance
        .collection('Poll')
        .where('locationData', isGreaterThan: GeoPoint(
  userLat - latIncrement(userLat, userLong),userLong - longIncrement(userLat, userLong),
)).where('locationData', isLessThan: GeoPoint(
  userLat + latIncrement(userLat, userLong),userLong + longIncrement(userLat, userLong),
    ...
    Future<void> fetchMore(int limit) async {
      ...
      final querySnapshot = await FirebaseFirestore.instance
        .collection('Poll')
         .where('locationData', isGreaterThan: GeoPoint(
  userLat - MAX_DISTANCE,userLong - MAX_DISTANCE,
